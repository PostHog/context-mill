...
Make **four `Agent` tool calls in a single message** so they run concurrently. Wait for all four to return, then continue to `5-report.md`. Do not run any other tools between dispatch and the next step.
