...
Wire up PostHog's AIO so calls made through {display_name} land in AIO as a full **session → trace → span → generation** tree — not just isolated `$ai_generation` events.
