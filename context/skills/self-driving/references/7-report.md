...
1. Write `./posthog-self-driving-report.md` (read any existing file first, then overwrite). Sections, in order:
   ...
   - **Products enabled** — from step 3b: a short table of Session Replay / Error Tracking / Support, each as enabled / already enabled / skipped (+ why, e.g. backend-only). For a web app, note whether the `posthog.init` override check was clean or edited. This is the *product* toggle, distinct from the signal sources below. **Support row:** Conversations is on, but tickets only arrive once an inbound channel is connected — spell out that the user must connect a channel (email / inbox / Slack) in PostHog, and add a matching follow-up.
