...
- **Stay off the internal surfaces.** Don't call `signals-scout-emit-signal` or any scratchpad-write tool, and don't change a scout's `emit` flag or `run_interval_minutes` — on configs, this skill only flips `enabled`. **Built-in scout bodies are never edited.** New scout skills are created in exactly one place: step 6b, and only ones the user approved there. Replay Vision scanners likewise come from exactly one place — step 6c's skeletons, whose locked fields (`scanner_type`, `emits_signals`, base prompt) you fill around, never rewrite.
