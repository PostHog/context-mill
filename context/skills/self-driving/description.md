...
- **Keep the scout troop small and inside the run budget.** Every enabled scout is a recurring LLM spend, and scout runs are budgeted server-side — a project gets up to 24 scout runs a day by default during early access (`scout-metadata-get` reports the enforced limits). Step 6 enables only `signals-scout-general` plus the **two or three** specialists for the products this project uses most — never error tracking or session replay (those reach the inbox as native sources) — and step 6b adds **at most two** custom scouts. Everything else stays disabled.
