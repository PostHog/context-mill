...
Read the .posthog-events.json file. For each event listed, use the PostHog MCP's `event-definition-update` tool to update the event definition with the description from the JSON file. This helps users understand what each event is for and why it was created.
...
- Uploading event definitions to PostHog
