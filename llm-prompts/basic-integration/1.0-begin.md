...
**IMPORTANT: Track actions only, not pageviews.** PostHog autocapture handles pageview tracking automatically. Adding manual pageview or page_viewed events creates duplicate data. Exceptions: "viewed"-type events that correspond to the top of a conversion funnel (e.g. pricing_page_viewed) are acceptable since they have semantic meaning beyond a raw pageview.
