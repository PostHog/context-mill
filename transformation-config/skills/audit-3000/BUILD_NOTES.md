...
- `references/8-use-case-match.md` — **new this session.** Reads `/tmp/co.json` + `/tmp/pe.json` from Step 7, scores PostHog's six use cases (product-intelligence, release-engineering, observability, growth-and-marketing, ai-llm-observability, data-infrastructure) using deterministic team/tag/title rules, applies archetype boost from Step 7's classification, picks a primary (score floor ≥3) and up to two secondaries, then **edits `/tmp/posthog-enrichment-staged.md` in place** to add a "Use case match" section between Company and Person. Reads `use-case-match-example.md` once to model output format. Skips silently when Step 7 produced no data. Does not touch the ledger.
- `references/9-use-case-expansion.md` — **rewritten 2026-05-13** as **expansion & cross-sell**. For each of 8 PostHog products (product analytics, error tracking, LLM observability, session replay, feature flags, surveys, logs, web analytics), runs TWO detectors in parallel (PostHog presence + competitor presence) and classifies into one of four modes per product: `cross-sell` (competitor detected, PostHog not used — pitch replacement), `greenfield` (nothing detected — pitch adoption), `gap` (PostHog used but surfaces missing — fill the gap), or `pass`. Competitor maps cover Sentry / Mixpanel / LogRocket / LaunchDarkly / Datadog / Langfuse / Hotjar / GA4 etc. with package names, source patterns, and env var names per competitor. **Dropped the ledger gate** that the original version had — Step 9 now always runs and writes findings to the ledger via `audit_resolve_checks`, same pattern as Steps 5/6. Step 10 reads the resolved entries and renders a dedicated "Use case expansion & cross-sell" section with three sub-tables (cross-sell, adoption, coverage gaps).
- `references/10-report.md` — final report writer. **2026-05-13 consolidation:** produces **exactly one file** at the project root (`posthog-audit-report.md`). Reads the ledger AND (when present) `/tmp/posthog-enrichment-staged.md`, inlines the staged enrichment as a `## Customer context` section and the use-case match as a `## Use case recommendation` section, renders the audit findings + the new `## Use case expansion & cross-sell` section + (when relevant) the **"Stale feature flag cleanup playbook"**. After writing, deletes both `/tmp/posthog-enrichment-staged.md` and `.posthog-audit-checks.json` in one Bash call.
