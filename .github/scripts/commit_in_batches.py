...
The target repo requires signed commits with no bypass actors, so commits are
...
Inputs (env): GH_TOKEN, TARGET_REPO (owner/repo), BRANCH, COMMIT_MESSAGE.
Reads the staged diff (git diff --cached) of the current working directory.
"""
